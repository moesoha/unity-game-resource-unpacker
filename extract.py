apkExtractedPath="C:\\Users\\soha\\com.papegames.evol_unpkg\\"
...
def parseCmdArgs(args):
	...
	if(args.indir=='' or args.outdir==''):
		...
		apkExtractedPath=args.indir
		assetsExtractTo=args.outdir
...
def handleFile(filepath):
	subdirname=filepath.replace(apkExtractedPath,"")
	savepath=os.path.join(assetsExtractTo,subdirname)
				...
				try:
					...
					# 	putFile(filename,savepath,data.script)
						...
						if isinstance(data.script,bytes):
							filename,mode=data.name+".bin","wb"
						else:
							filename,mode=data.name+".txt","w"
						putFile(filename,savepath,data.script,mode=mode)
				except Exception as e:
					print("WARNING: Error while processing %r (%s)"%(filepath,e))

def main():
	for currentPath,dirs,files in os.walk(apkExtractedPath):
